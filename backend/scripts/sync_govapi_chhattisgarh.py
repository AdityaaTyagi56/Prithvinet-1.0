...
# Allow importing app.services from the backend root
sys.path.insert(0, str(Path(__file__).resolve().parents[1]))
from app.services.aqi_csv_logger import append_readings_to_csv
...
def _load_env() -> Tuple[str, str]:
    ...
    if not database_url:
        raise SyncError(
            ...
            "Add it to your .env file: DATABASE_URL=postgresql+asyncpg://user:pass@host:5432/dbname"
        )
    if not govapi_key:
        logging.warning(
            "GOVAPI_KEY is not set. "
            "Register at https://api.data.gov.in/ to get a free API key, "
            "then add GOVAPI_KEY=<your-key> to your .env file. "
...
def _normalize_record(record: Dict[str, Any]) -> Dict[str, Optional[float]]:
    """Extract pollutant values from a GovAPI record.

    The API returns ONE record per pollutant per station with fields:
        pollutant_id: "PM10" / "SO2" / "NO2" / …
        avg_value / min_value / max_value: numeric strings
    If the record instead has column-style fields (pm10, so2, …) we
    ...
    # ---- primary path: per-pollutant record (pollutant_id + avg_value) ----
    pid = str(record.get("pollutant_id", "")).strip().upper()
    if pid:
        # Map common API pollutant_id values to our canonical names
        pid_map = {
            "PM10": "PM10",
            "PM2.5": "PM2.5",
            "PM25": "PM2.5",
        ...
        canonical = pid_map.get(pid)
        if canonical:
            val = as_float(record.get("avg_value") or record.get("min_value"))
            if val is not None:
                out[canonical] = val
        return out

    # ---- fallback: column-style record (pm10, so2 as keys) ----
...
def _fetch_govapi_curl(govapi_key: str, limit: int = 500, offset: int = 0) -> Dict[str, Any]:
    """Fetch via curl subprocess — reliable on Windows where Python SSL can timeout."""
    ...
    if result.returncode != 0:
        raise SyncError(f"curl failed (rc={result.returncode}): {result.stderr[:200]}")
    if not result.stdout.strip():
        raise SyncError("curl returned empty response")
...
async def _fetch_govapi(govapi_key: str) -> List[Dict[str, Any]]:
    ...
    # Try httpx first; fall back to curl on timeout (common on Windows)
    use_curl = False
    try:
        ...
        async with httpx.AsyncClient(timeout=timeout) as client:
            ...
            if resp.status_code in (401, 403):
                raise SyncError(
                    f"data.gov.in API returned HTTP {resp.status_code}. "
                    "Check your GOVAPI_KEY at https://api.data.gov.in/"
                )
            resp.raise_for_status()
            payload = resp.json()
            batch = payload.get("records") or []
            all_records.extend(batch)
            logging.info("Fetched %d records via httpx", len(batch))
    except (httpx.TimeoutException, httpx.ConnectError) as e:
        logging.warning("httpx failed (%s), falling back to curl", e)
        use_curl = True
    ...
    if use_curl:
        ...
        offset = 0
        while True:
            payload = _fetch_govapi_curl(govapi_key, limit=limit, offset=offset)
            batch = payload.get("records") or []
            if not isinstance(batch, list) or not batch:
                break
            all_records.extend(batch)
            logging.info("Fetched %d records via curl (offset=%d)", len(batch), offset)
            if len(batch) < limit:
                break
            offset += limit
...
async def sync_once(session: AsyncSession, govapi_key: str) -> None:
    if not govapi_key:
        logging.warning(
            "Skipping GovAPI sync: GOVAPI_KEY not configured. "
            "Get a free key at https://api.data.gov.in/ and set GOVAPI_KEY in .env"
        )
        return
    ...
    # Group csv_rows by station+timestamp for a cleaner spreadsheet
    # (API returns one row per pollutant; we merge into one row per station)
    merged_csv: Dict[str, Dict[str, Any]] = {}
    for row in csv_rows:
        key = f"{row['station_name']}|{row['timestamp']}"
        if key not in merged_csv:
            merged_csv[key] = {
        ...
        for p in ("PM10", "PM2.5", "SO2", "NO2"):
            if row.get(p) not in (None, ""):
                merged_csv[key][p] = row[p]

    final_csv_rows = list(merged_csv.values())

    # Write newly inserted readings to daily CSV spreadsheet
    if final_csv_rows:
        try:
            append_readings_to_csv(final_csv_rows)
        except Exception as e:
            logging.warning("CSV logging failed (non-fatal): %s", e)

    logging.info(
        "Sync complete: inserted=%d skipped=%d districts=%d csv_rows=%d source=govapi",
        inserts,
        skipped,
        len(districts_seen),
        len(csv_rows),
