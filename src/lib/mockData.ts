...
export function getAlerts(): AlertData[] {
  const now = new Date();
  return [
    { id: 'alrt-1', pollution_type: 'air', location: 'Bhilai Steel Plant Gate', region: 'Durg', industry: 'Bhilai Steel Plant (SAIL)', parameter: 'SO2', value: 94.2, threshold: 80, severity: 'critical', status: 'active', triggered_at: new Date(now.getTime() - 25 * 60000).toISOString(), auto_escalation_at: new Date(now.getTime() + 35 * 60000).toISOString(), recommended_action: 'Activate scrubber system on BF#3; deploy mobile monitoring at downwind residential zone. Source: CECB OCEMS — Bhilai Steel Stack 2' },
    { id: 'alrt-2', pollution_type: 'air', location: 'NTPC Korba STPS', region: 'Korba', industry: 'NTPC Korba Super Thermal Power Station', parameter: 'PM10', value: 148.6, threshold: 100, severity: 'high', status: 'escalated', triggered_at: new Date(now.getTime() - 120 * 60000).toISOString(), auto_escalation_at: null, recommended_action: 'Issue show-cause notice; verify ESP efficiency; schedule inspection within 48h. Source: CPCB RTDMS Live' },
    { id: 'alrt-3', pollution_type: 'air', location: 'NTPC Sipat TPS', region: 'Bilaspur', industry: 'NTPC Sipat Thermal Power Station', parameter: 'NOx', value: 342.1, threshold: 300, severity: 'medium', status: 'active', triggered_at: new Date(now.getTime() - 45 * 60000).toISOString(), auto_escalation_at: new Date(now.getTime() + 15 * 60000).toISOString(), recommended_action: 'Check combustion temperature and excess-air ratio; reduce fuel feed rate 15%. CPCB limit: 300 mg/Nm3. Source: CECB OCEMS' },
    { id: 'alrt-4', pollution_type: 'air', location: 'Nova Iron & Steel Factory', region: 'Bilaspur', industry: 'Nova Iron & Steel Bilaspur', parameter: 'PM', value: 2292, threshold: 150, severity: 'critical', status: 'escalated', triggered_at: new Date(now.getTime() - 180 * 60000).toISOString(), auto_escalation_at: null, recommended_action: 'HISTORICAL VIOLATION: SPM 2292 mg/m³ (15× CPCB limit of 150). Documented in CSE Inspection Report June 2009. Immediate closure direction recommended under EP Act 1986.' },
    { id: 'alrt-5', pollution_type: 'water', location: 'Kharoon Nallah — Raipur Outfall', region: 'Raipur', industry: 'Industrial Cluster Siltara', parameter: 'BOD', value: 48.5, threshold: 30, severity: 'critical', status: 'active', triggered_at: new Date(now.getTime() - 15 * 60000).toISOString(), auto_escalation_at: new Date(now.getTime() + 45 * 60000).toISOString(), recommended_action: 'Immediate sample collection; notify SPCB; inspect CETP operations. Source: CPCB NWMP — Kharoon River Bundri' },
    { id: 'alrt-6', pollution_type: 'water', location: 'Dengur Nallah at Korba', region: 'Korba', industry: 'CSEB Korba West Power Company', parameter: 'DO', value: 3.1, threshold: 6.0, severity: 'critical', status: 'active', triggered_at: new Date(now.getTime() - 60 * 60000).toISOString(), auto_escalation_at: new Date(now.getTime()).toISOString(), recommended_action: 'Critically low dissolved oxygen from thermal plant effluent discharge. Halt discharge immediately; collect upstream/downstream samples. Source: CPCB NWMP verified baseline' },
    { id: 'alrt-7', pollution_type: 'water', location: 'Seonath River — Durg Bridge', region: 'Durg', industry: 'Bhilai Steel Plant (SAIL)', parameter: 'COD', value: 310, threshold: 250, severity: 'medium', status: 'acknowledged', triggered_at: new Date(now.getTime() - 200 * 60000).toISOString(), auto_escalation_at: null, recommended_action: 'Verify treatment plant operations; schedule follow-up sampling. Source: CPCB NWMP' },
    { id: 'alrt-8', pollution_type: 'noise', location: 'Korba Industrial Zone', region: 'Korba', industry: 'NTPC Korba + BALCO Complex', parameter: 'Leq', value: 82.4, threshold: 75, severity: 'high', status: 'active', triggered_at: new Date(now.getTime() - 30 * 60000).toISOString(), auto_escalation_at: new Date(now.getTime() + 30 * 60000).toISOString(), recommended_action: 'Verify noise barriers; issue direction under Noise Pollution Rules 2000. Source: CG research baseline + NANMN pattern' },
    { id: 'alrt-9', pollution_type: 'noise', location: 'Bhilai — Sector 6 Market', region: 'Durg', industry: 'Bhilai Steel Plant (SAIL)', parameter: 'Lmax', value: 92.1, threshold: 85, severity: 'critical', status: 'escalated', triggered_at: new Date(now.getTime() - 90 * 60000).toISOString(), auto_escalation_at: null, recommended_action: 'Night-time operations exceeding limits; issue closure direction for non-compliant unit. Source: CG research baseline' },
    { id: 'alrt-10', pollution_type: 'noise', location: 'Rajnandgaon — Hospital Zone', region: 'Rajnandgaon', industry: 'Construction Site', parameter: 'Leq', value: 71.8, threshold: 50, severity: 'high', status: 'active', triggered_at: new Date(now.getTime() - 40 * 60000).toISOString(), auto_escalation_at: new Date(now.getTime() + 20 * 60000).toISOString(), recommended_action: 'Silence zone violation; halt noisy construction; notify district administration. CPCB Noise Standard: 50 dB(A) day, 40 dB(A) night.' },
  ];
}
...
export function getIndustryTracker(): IndustryData[] {
  return [
    { id: 'ind-1', name: 'Bhilai Steel Plant (SAIL)', type: 'Integrated Steel', region: 'Durg', consent_valid_until: '2026-12-31', air_status: 'non-compliant', water_status: 'warning', noise_status: 'non-compliant', last_inspection: '2026-02-15', total_violations_ytd: 8, risk_score: 88 },
    { id: 'ind-2', name: 'NTPC Korba Super Thermal Power Station', type: 'Thermal Power Plant', region: 'Korba', consent_valid_until: '2027-03-31', air_status: 'non-compliant', water_status: 'compliant', noise_status: 'compliant', last_inspection: '2026-01-20', total_violations_ytd: 5, risk_score: 92 },
    { id: 'ind-3', name: 'NTPC Sipat Thermal Power Station', type: 'Thermal Power Plant', region: 'Bilaspur', consent_valid_until: '2027-06-30', air_status: 'warning', water_status: 'compliant', noise_status: 'compliant', last_inspection: '2026-02-28', total_violations_ytd: 3, risk_score: 91 },
    { id: 'ind-4', name: 'CSEB Korba West Power Company', type: 'Thermal Power Plant', region: 'Korba', consent_valid_until: '2026-09-30', air_status: 'non-compliant', water_status: 'warning', noise_status: 'compliant', last_inspection: '2026-03-01', total_violations_ytd: 6, risk_score: 90 },
    { id: 'ind-5', name: 'Vedanta Aluminium Korba (BALCO)', type: 'Aluminium Smelter', region: 'Korba', consent_valid_until: '2027-01-15', air_status: 'warning', water_status: 'compliant', noise_status: 'warning', last_inspection: '2026-02-10', total_violations_ytd: 4, risk_score: 85 },
    { id: 'ind-6', name: 'ACC Cement Jamul', type: 'Cement', region: 'Durg', consent_valid_until: '2026-08-31', air_status: 'compliant', water_status: 'n/a', noise_status: 'warning', last_inspection: '2026-01-05', total_violations_ytd: 2, risk_score: 72 },
    { id: 'ind-7', name: 'UltraTech Cement Hirmi', type: 'Cement', region: 'Raipur', consent_valid_until: '2026-10-31', air_status: 'compliant', water_status: 'n/a', noise_status: 'compliant', last_inspection: '2026-02-22', total_violations_ytd: 1, risk_score: 70 },
    { id: 'ind-8', name: 'Monnet Ispat & Energy Raigarh', type: 'Sponge Iron', region: 'Raigarh', consent_valid_until: '2026-06-30', air_status: 'non-compliant', water_status: 'warning', noise_status: 'non-compliant', last_inspection: '2026-03-05', total_violations_ytd: 7, risk_score: 82 },
    { id: 'ind-9', name: 'Jindal Steel & Power Raigarh', type: 'Integrated Steel', region: 'Raigarh', consent_valid_until: '2027-02-28', air_status: 'non-compliant', water_status: 'compliant', noise_status: 'warning', last_inspection: '2026-02-18', total_violations_ytd: 5, risk_score: 86 },
    { id: 'ind-10', name: 'Nova Iron & Steel Bilaspur', type: 'Sponge Iron', region: 'Bilaspur', consent_valid_until: '2026-04-15', air_status: 'non-compliant', water_status: 'n/a', noise_status: 'non-compliant', last_inspection: '2026-01-25', total_violations_ytd: 9, risk_score: 78 },
  ];
}
