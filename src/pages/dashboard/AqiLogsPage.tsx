...
import { hasLiveData, getCachedSnapshot, fetchLiveSnapshot, triggerBackendSync } from '../../lib/liveData';
...
export function AqiLogsPage() {
  ...
  const [lastSyncIST, setLastSyncIST] = useState<string | null>(null);

  const [snapshotObj, setSnapshotObj] = useState(getCachedSnapshot());
  const snap = snapshotObj;

  // Auto-sync with govt API on page mount and refresh snapshot every 30s
  useEffect(() => {
    // Trigger backend sync on mount (pulls latest hourly data from data.gov.in)
    setSyncing(true);
    triggerBackendSync()
      .then((res) => {
        if (res?.synced) {
          setLastSyncIST(res.sync_time_ist || null);
        } else if (res?.last_sync_ist) {
          setLastSyncIST(res.last_sync_ist);
        }
      })
      .catch(() => {})
  ...
  const handleManualSync = async () => {
    setSyncing(true);
    try {
      const res = await triggerBackendSync();
      if (res?.synced) {
        setLastSyncIST(res.sync_time_ist || null);
        // Re-fetch log list and current date rows
        const logRes = await api.get('/aqi-logs/');
        setLogs(logRes.data.logs || []);
        if (selectedDate) {
          const rowRes = await api.get(`/aqi-logs/${selectedDate}`);
          setRows(rowRes.data.rows || []);
        }
      } else if (res?.reason) {
        setLastSyncIST(res.last_sync_ist || lastSyncIST);
      }
      const fresh = await fetchLiveSnapshot(true);
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            AQI readings from CPCB Government API (data.gov.in) — All times in IST
            {hasLiveData() && (
          ...
          {(lastSyncIST || fetchedAt) && (
            <p className="text-xs text-gray-400 mt-1 flex items-center gap-2">
              <Clock className="h-3 w-3" />
              Last synced: {lastSyncIST || (fetchedAt ? formatIST(fetchedAt) : '—')}
              {syncing && <RefreshCw className="h-3 w-3 animate-spin text-green-600" />}
            </p>
          )}
        </div>
        <div className="flex gap-2">
          <button
            ...
            disabled={syncing}
            className="flex items-center gap-1.5 px-3 py-1.5 bg-white border border-green-300 rounded-lg text-sm font-medium text-green-700 hover:bg-green-50 transition-colors shadow-sm disabled:opacity-50"
          >
                    ...
                    return (
                      <div key={fp.label} className="border border-gray-200 rounded-lg p-3 text-center">
                        ...
                        <div className="text-xs text-gray-500 mb-2">
                          {new Date(fp.time).toLocaleString('en-IN', { hour: '2-digit', minute: '2-digit', day: 'numeric', month: 'short', timeZone: 'Asia/Kolkata' })}
                        </div>
