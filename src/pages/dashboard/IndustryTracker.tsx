...
function categoryBadge(type: string) {
  ...
  const cls = cat === 'RED' ? 'bg-red-100 text-red-700 border-red-300' : 'bg-orange-100 text-orange-700 border-orange-300';
  return <span className={`inline-block text-[9px] font-bold px-1.5 py-0.5 rounded border ${cls}`}>{cat}</span>;
}
...
function rankBadge(rank: number) {
  if (rank <= 3) {
    const bg = rank === 1 ? 'bg-red-600' : rank === 2 ? 'bg-red-500' : 'bg-orange-500';
    return <span className={`inline-flex items-center justify-center w-6 h-6 rounded-full ${bg} text-white text-xs font-bold`}>#{rank}</span>;
  }
...
// Extended with local action state
...
function IndustryDetailPanel({
  industry, onScheduleInspection, onIssueNotice, onRenewConsent,
}: {
  ...
  const scores = [industry.air_status, industry.water_status, industry.noise_status]
    .filter(s => s !== 'n/a' && scoreMap[s] !== undefined)
    .map(s => scoreMap[s]);
  const compPct = scores.length > 0 ? Math.round(scores.reduce((a, b) => a + b, 0) / scores.length) : 100;
  const compColor = compPct >= 80 ? 'text-green-600' : compPct >= 40 ? 'text-amber-600' : 'text-red-600';
  ...
  return (
    <div className="px-6 pb-5 pt-2 bg-slate-50/50 border-t border-slate-200">
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-4">
          ...
          <div className="space-y-2">
            ...
            {Object.keys(limits).length === 0 && <div className="text-xs text-gray-400">No specific limits</div>}
          </div>
          <div className="mt-3 text-[9px] text-gray-400">Ref: EP Rules 1986 Schedule-I — {industry.type}</div>
        </div>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-4">
          ...
          <div className="space-y-2">
            {[['Air', industry.air_status], ['Water', industry.water_status], ['Noise', industry.noise_status]].map(([label, st]) => (
              <div key={label as string} className="flex justify-between items-center">
                <span className="text-sm text-gray-600">{label}</span>
                {statusBadge(st as string)}
              </div>
            ))}
          </div>
          <div className="mt-3 pt-2 border-t border-gray-100 flex justify-between items-center">
            <span className="text-xs text-gray-500">Overall Compliance</span>
            <span className={`text-lg font-bold ${compColor}`}>{compPct}%</span>
          </div>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-4">
          ...
          <div className="space-y-2 text-sm">
            <div className="flex justify-between"><span className="text-gray-600">Category</span>{categoryBadge(industry.type)}</div>
            <div className="flex justify-between"><span className="text-gray-600">Type</span><span className="font-medium text-gray-800 text-xs">{industry.type}</span></div>
            <div className="flex justify-between"><span className="text-gray-600">Region</span><span className="font-medium text-gray-800">{industry.region}</span></div>
            <div className="flex justify-between"><span className="text-gray-600">Consent</span>{consentStatus(industry.consent_valid_until)}</div>
            <div className="flex justify-between">
              <span className="text-gray-600">Last Inspection</span>
              <span className="text-xs text-gray-500">{new Date(industry.last_inspection).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-4">
          ...
          <div className="space-y-2">
            {industry.inspection_scheduled ? (
              <div className="bg-blue-50 border border-blue-200 rounded px-3 py-2 flex items-center gap-2">
                ...
                <span className="text-xs text-blue-700 font-medium">
                  Inspection: {new Date(industry.inspection_scheduled).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}
                </span>
              </div>
            ) : (
              <button onClick={(e) => { e.stopPropagation(); onScheduleInspection(industry.id); }}
                ...
                <Calendar className="h-3.5 w-3.5" /> Schedule Inspection
              </button>
            )}
            ...
            {industry.notice_issued ? (
              ...
              <button onClick={(e) => { e.stopPropagation(); onIssueNotice(industry.id); }}
                className="w-full bg-amber-600 text-white px-3 py-2 rounded text-xs font-medium hover:bg-amber-700 transition-colors flex items-center gap-2 justify-center disabled:opacity-50"
                disabled={industry.total_violations_ytd < 3}
                title={industry.total_violations_ytd < 3 ? 'Requires 3+ violations YTD' : ''}>
                <FileText className="h-3.5 w-3.5" /> Issue Show-Cause Notice
                {industry.total_violations_ytd < 3 && <span className="text-[9px] opacity-70">(Need 3+)</span>}
              </button>
            )}

            {new Date(industry.consent_valid_until).getTime() < Date.now() + 90 * 86400000 && !industry.consent_renewed ? (
              <button onClick={(e) => { e.stopPropagation(); onRenewConsent(industry.id); }}
                ...
                <CheckCircle2 className="h-3.5 w-3.5" /> Initiate Consent Renewal
              </button>
          ...
          {industry.risk_score >= 85 && (
            <div className="mt-3 bg-red-50 border border-red-200 rounded px-3 py-2">
              <p className="text-[10px] text-red-700 font-medium">
                High-risk industry. Consider closure direction under EP Act 1986 S.5.
              </p>
            </div>
          )}
        </div>
      ...
      {/* Nova Iron historical violation */}
...
export function IndustryTracker() {
  ...
  const logAction = useCallback((action: string) => {
    setActionLog(prev => [{ action, time: new Date().toISOString() }, ...prev.slice(0, 49)]);
    setToast(action);
  }, []);

  const handleScheduleInspection = useCallback((id: string) => {
    const scheduled = new Date(Date.now() + 14 * 86400000).toISOString();
  ...
  const handleRenewConsent = useCallback((id: string) => {
    ...
    const ind = industries.find(i => i.id === id);
    logAction(`Consent renewal initiated for ${ind?.name}`);
  }, [industries, logAction]);
  ...
  const filtered = useMemo(() => {
    return industries
      .filter(i => {
        ...
        if (searchQuery && !`${i.name} ${i.type} ${i.region}`.toLowerCase().includes(searchQuery.toLowerCase())) return false;
        return true;
      })
      .sort((a, b) => {
  ...
  }, [industries, filterRegion, filterCompliance, searchQuery, sortBy]);

  const totalNonCompliant = industries.filter(i => i.air_status === 'non-compliant' || i.water_status === 'non-compliant' || i.noise_status === 'non-compliant').length;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        <div className="gov-card p-4 text-center cursor-pointer hover:shadow-md transition-shadow" onClick={() => { setFilterCompliance('all'); setFilterRegion('all'); setSearchQuery(''); }}>
      ...
      <div className="gov-card overflow-hidden">
          ...
          <div className="flex gap-2 flex-wrap">
            <div className="relative">
              ...
              <input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
                placeholder="Search industry..." className="pl-6 rounded border border-white/30 px-2 py-1 text-xs bg-white/10 text-white placeholder-white/40 w-40" />
            </div>
            <select value={filterCompliance} onChange={e => setFilterCompliance(e.target.value as any)} className="rounded border border-white/30 px-2 py-1 text-xs bg-white/10 text-white">
            ...
            <select value={filterRegion} onChange={e => setFilterRegion(e.target.value)} className="rounded border border-white/30 px-2 py-1 text-xs bg-white/10 text-white">
            ...
            <select value={sortBy} onChange={e => setSortBy(e.target.value as any)} className="rounded border border-white/30 px-2 py-1 text-xs bg-white/10 text-white">
        ...
        <div className="overflow-x-auto">
          <table className="gov-table">
            ...
            <tbody>
              {filtered.length === 0 && (
                <tr><td colSpan={12} className="text-center py-8 text-gray-400 text-sm">
                  {searchQuery ? `No industries found for "${searchQuery}"` : 'No industries match the selected filters.'}
                </td></tr>
              )}
              {filtered.map((ind, idx) => {
                const isExpanded = expandedId === ind.id;
                const hasViolation = ind.air_status === 'non-compliant' || ind.water_status === 'non-compliant' || ind.noise_status === 'non-compliant';
                return (
                  <React.Fragment key={ind.id}>
                    <tr className={`cursor-pointer hover:bg-gray-50 transition-colors ${ind.risk_score >= 90 ? 'bg-red-50/60' : ind.risk_score >= 80 ? 'bg-red-50/30' : ''}`}
                      ...
                      <td>
                        <div className="flex items-center gap-1">
                          ...
                          {ind.name.includes('Nova Iron') && <AlertTriangle className="h-3.5 w-3.5 text-red-500 flex-shrink-0" title="Historical critical violator" />}
                          {ind.notice_issued && <span className="text-[8px] bg-amber-100 text-amber-700 border border-amber-200 px-1 py-0.5 rounded font-bold flex-shrink-0">SCN</span>}
                          {ind.inspection_scheduled && <Calendar className="h-3 w-3 text-blue-500 flex-shrink-0" title={`Inspection scheduled`} />}
                        </div>
                      </td>
                      <td className="text-xs text-gray-500">{ind.type}</td>
                      <td className="text-sm"><span className="flex items-center gap-1"><MapPin className="h-3 w-3 text-gray-400" />{ind.region}</span></td>
                      ...
                      <td className="text-center">
                        {ind.total_violations_ytd > 0 ? <span className="badge-high">{ind.total_violations_ytd}</span> : <span className="badge-low">0</span>}
                      </td>
                      <td>{cemsIndicator(hasViolation)}</td>
        ...
        <div className="px-5 py-3 bg-gray-50 border-t border-gray-200 flex items-center justify-between">
          <div className="text-[10px] text-gray-400">
            EP Rules 1986 Schedule-I · CPCB Classification 2016 · CECB OCEMS · CPCB RTDMS
          </div>
          <div className="text-[10px] text-gray-400">{filtered.length} of {industries.length} industries</div>
        </div>
      </div>
      ...
      {actionLog.length > 0 && (
        <div className="gov-card overflow-hidden">
          <div className="gov-card-header flex items-center justify-between">
            <span className="flex items-center gap-2"><ClipboardList className="h-4 w-4" /> Regulatory Actions Log</span>
          ...
          <div className="divide-y divide-gray-100 max-h-48 overflow-y-auto">
            {actionLog.map((entry, idx) => (
              <div key={idx} className="px-4 py-2 flex items-center gap-3 text-xs">
                <span className="text-gray-400 flex-shrink-0">{new Date(entry.time).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit', second: '2-digit' })}</span>
